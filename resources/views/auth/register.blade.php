        ...
        <section class="flex flex-col gap-6 p-8 w-96 bg-white rounded-lg shadow-md max-sm:w-full">
            ...
            <form class="flex flex-col gap-6" action="{{ route('register') }}" method="POST">
                ...
                <div class="flex flex-col gap-2">
                    <label for="name" class="h-5 text-sm font-bold leading-5 text-gray-700">
                        Name
                    </label>
                    <input type="text" name="name" id="name" placeholder="Enter your name" class="px-3.5 py-2 w-full text-base bg-white rounded border shadow h-[38px] text-zinc-400" />
                </div>
                <div class="flex flex-col gap-2">
